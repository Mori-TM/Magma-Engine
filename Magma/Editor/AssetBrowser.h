		...
		//	ImGui::BeginChild("Asset Pop Up Child");
		{
			if (ImGui::BeginPopupContextWindow("Asset Pop Up"))
			{
				...
				if (ImGui::Selectable("Load Model"))
				{
					...
					if (WaveOpenFileDialog(Path, "All Models\0*.obj;*.dae;*.stl\0"))
					{
						...
						if (ModelData.VertexCount > 0)
							if (LoadModel(Path, &ModelData, &MeshInfo))
								AddMesh(GetFileNameFromPath(Path), &MeshInfo);
							else
								printf("Failed to create vertex buffer from model\n");
						else
				...
				if (ImGui::Selectable("Load Texture"))
				{
					...
					if (WaveOpenFileDialog(Path, "All Images\0*.png;*.jpg;*.tga;*.jpeg;*.hdr;*.bmp\0"))
						AddTexture(Path, true);
					WaveResetToLastPath();
				}
