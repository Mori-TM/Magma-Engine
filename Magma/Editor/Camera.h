...
void UpdateFpsCamera(vec3 *CameraPos, vec3 *CameraFront, vec3 *CameraUp) {
    ...
    if (State[SDL_SCANCODE_LSHIFT]) CameraInfo.DeltaTime *= 0.03;
    else CameraInfo.DeltaTime *= 0.01;
    ...
    if (MouseState == 4 || CameraInfo.FirstInit) {
        ...
        if (State[SDL_SCANCODE_W]) *CameraPos = Add3(*CameraPos, Mul3P(&CamerSpeed, CameraFront));

        if (State[SDL_SCANCODE_S]) *CameraPos = Sub3(*CameraPos, Mul3P(&CamerSpeed, CameraFront));

        if (State[SDL_SCANCODE_A]) {
            *CameraPos = Sub3(*CameraPos, Mul3(CamerSpeed, Normalize3(Cross3P(CameraFront, CameraUp))));
        }

        if (State[SDL_SCANCODE_D]) *CameraPos = Add3(*CameraPos, Mul3(CamerSpeed, Normalize3(Cross3P(CameraFront, CameraUp))));
        ...
        } else CameraInfo.DeltaX = CameraInfo.DeltaY = 0.0;
        ...
        if (CameraInfo.Pitch >= 89.0) CameraInfo.Pitch = 89.0;
