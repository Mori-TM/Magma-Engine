        ...
        if (EntityCount > 0) {
            ...
            if (ImGui::BeginPopupContextWindow("Entity Inspector Pop Up")) {
                ...
                for (uint32_t i = 0; i < COMPONENT_COUNT; i++) {
                    if (ImGui::Button(Components[i])) Entities[SelectedEntity].UsedComponents[i] = true;
                }
...
void EditorTextureCombo(const char *Name, const char *ID, uint32_t *TextureIndex) {
    ...
    if (ImGui::BeginCombo(Name, Image->Name)) {
        for (uint32_t i = 0; i < SceneTextures.Size; i++) {
            ...
            if (Image != NULL) {
                if (ImGui::ImageButton(&GetDescriptorSet(Image->TextureDescriptorSet)[0], ImVec2(30, 30))) *TextureIndex = i;
            ...
            if (Material != NULL) {
                ...
                if (ImGui::Button("Delete Material")) {
                    for (uint32_t i = 0; i < EntityCount; i++) {
                        if (Entities[i].UsedComponents[COMPONENT_TYPE_MATERIAL] && Entities[i].Material.MaterialIndex == SelectedMaterial) ResetEntityMaterial(&Entities[i]);
                    }

                    CMA_Pop(&SceneMaterials, SelectedMaterial);

                    for (uint32_t i = 1; i < SceneMaterials.Size; i++) {
                        if (CMA_GetAt(&SceneMaterials, i) != NULL) SelectedMaterial = i;
                    }
                }
            ...
            if (Image != NULL) {
                ...
                if (ImGui::Button("Delete Texture")) {
                    for (uint32_t i = 0; i < SceneMaterials.Size; i++) {
                        ...
                        if (Material != NULL) {
                            if (Material->AlbedoIndex == SelectedTexture) Material->AlbedoIndex = 0;
                            else if (Material->NormalIndex == SelectedTexture) Material->NormalIndex = 0;
                            else if (Material->MetallicIndex == SelectedTexture) Material->MetallicIndex = 0;
                            else if (Material->RoughnessIndex == SelectedTexture) Material->RoughnessIndex = 0;
                            else if (Material->OcclusionIndex == SelectedTexture) Material->OcclusionIndex = 0;
                        }
                    }
                    ...
                    for (uint32_t i = 1; i < SceneTextures.Size; i++) {
                        if (CMA_GetAt(&SceneTextures, i) != NULL) SelectedTexture = i;
                    }
    ...
    ImGui::Begin("Mesh Inspector");
    {
        if (SceneMeshes.Size > 1) {
            ...
            if (ImGui::Button("Delete Mesh")) {
                for (uint32_t i = 0; i < EntityCount; i++) {
                    if (Entities[i].UsedComponents[COMPONENT_TYPE_MESH] && Entities[i].Mesh.MeshIndex == SelectedMesh) ResetEntityMesh(&Entities[i]);
                }
...
void EditorVSInspector() {
    ImGui::Begin("VS Inspector");
    {
        ...
        for (int n = 0; n < points.Size; n += 2) {
            ...
            if (point != NULL && point2 != NULL) draw_list->AddLine(ImVec2(origin.x + point->x, origin.y + point->y), ImVec2(origin.x + point2->x, origin.y + point2->y), IM_COL32(255, 255, 0, 255), 2.0f);
        }
