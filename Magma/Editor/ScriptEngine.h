...
int32_t GetDownKey(lua_State *LuaState) {
    ...
    if (IsString || IsNumber) {
        ...
        if (IsString) Key = lua_tostring(LuaState, 1)[0];

        if (IsNumber) Key = lua_tonumber(LuaState, 1);

        const Uint8 *State = SDL_GetKeyboardState(NULL);

        if (State[SDL_GetScancodeFromKey(Key)]) lua_pushboolean(LuaState, true);
    }
...
void LuaReloadFiles() {
    if (!(SDL_GetWindowFlags(Window) & SDL_WINDOW_INPUT_FOCUS)) WindowWasFocused = true;
    ...
    if (WindowWasFocused && SDL_GetWindowFlags(Window) & SDL_WINDOW_INPUT_FOCUS) {
        ...
        for (uint32_t i = 0; i < SceneScriptCount; i++) {
            size_t Size;
            strcpy(SceneScripts[SelectedScript].Script, OpenVkReadFileData(SceneScripts[SelectedScript].Path, &Size));
        }
