...
void ShadowDraw() {
    ...
    OpenVkBeginRenderPass(&BeginInfo);
    {
        ...
        for (uint32_t j = 0; j < SHADOW_MAP_CASCADE_COUNT; j++) {
            ...
            if (ShadowBackfaceCulling) Pipeline = ShadowPipelineBackCull;
            ...
            for (uint32_t i = 0; i < EntityCount; i++) {
                ...
                if (Material == NULL) Material = (SceneMaterial *)CMA_GetAt(&SceneMaterials, 0);

                SceneTextureImage *Image;

                if (Entities[i].UsedComponents[COMPONENT_TYPE_MATERIAL]) Image = (SceneTextureImage *)CMA_GetAt(&SceneTextures, Material->AlbedoIndex);
                else Image = (SceneTextureImage *)CMA_GetAt(&SceneTextures, 0);

                uint32_t TextureDescriptorSet = 0;

                if (Image != NULL) TextureDescriptorSet = Image->TextureDescriptorSet;
