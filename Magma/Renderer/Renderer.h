...
void RendererEvent() {
    ...
    if (Event.type == SDL_KEYDOWN && Event.key.keysym.sym == SDLK_F11) SDL_SetWindowFullscreen(Window, (FullScreen = !FullScreen) ? SDL_WINDOW_FULLSCREEN_DESKTOP : false);
    ...
    if (DeleteMesh) {
        ...
        if (Mesh != NULL && Mesh->Freeable) {
            for (uint32_t i = 0; i < Mesh->MeshCount; i++) {
                ...
                if (Mesh->MeshData[i].Vertices != NULL) OpenVkDestroyBuffer(Mesh->MeshData[i].VertexBuffer);

                if (Mesh->MeshData[i].Indices != NULL) OpenVkDestroyBuffer(Mesh->MeshData[i].IndexBuffer);
            }
        }
        ...
        for (uint32_t i = 1; i < SceneMeshes.Size; i++) {
            if (CMA_GetAt(&SceneMeshes, i) != NULL) SelectedMesh = i;
        }
