...
namespace ifd {
	struct ImageData
	{
		...
		bool operator==(const ImageData& other) const {
			return (this->DescriptorSet == other.DescriptorSet);
		}
	};
	...
	public:
		...
		std::function<ImageData(uint8_t*, int, int, char)> CreateTexture; // char -> fmt -> { 0 = BGRA, 1 = RGBA }
		std::function<void(ImageData)> DeleteTexture;
	...
	private:
		...
		std::unordered_map<std::string, ImageData> m_icons;
		ImageData m_getIcon(const std::filesystem::path& path);
		ImageData m_getIconNew(std::string Tmp);
		void m_clearIcons();
