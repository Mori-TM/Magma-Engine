...
void OpenVkGUIInit(uint32_t Width, uint32_t Height, uint32_t RenderPass, uint32_t MsaaSamples, float FontSize, const char* Font, uint32_t(*GetMousePos)(int32_t* x, int32_t* y))
{
	...
	GraphicsPipelineCreateInfo.DepthWrite = OpenVkTrue;//doesn't matter
	GraphicsPipelineCreateInfo.DepthCompare = OPENVK_DEPTH_COMPARE_LESS_OR_EQUAL;//doesn't matter
...
void OpenVkGUISortIndices()
{
	for (uint32_t i = 0; i < OpenVkGUI.WindowCount; i++)
	{
		OpenVkGUIWindow* Wnd = &OpenVkGUI.Windows[i];
...
int32_t OpenVkGUIQuadPos(float x, float y, float w, float h, float Rounding, float Color)
{
	OpenVkGUIVertex Vertices[4] =
	{
...
void OpenVkGUIQuadPosExt(OpenVkGUIQuad* Quad)
{
	...
	OpenVkGUIVertex Vertices[4] =
	{
		{ { (float)Quad->x + Quad->w, (float)Quad->y + Quad->h },	{1001.0, 1001.0}, {(float)Quad->w, (float)Quad->h, Quad->Rounding, Quad->Color} },
		{ { (float)Quad->x + Quad->w, (float)Quad->y			 },	{1001.0,  999.0},  {(float)Quad->w, (float)Quad->h, Quad->Rounding, Quad->Color} },
		{ { (float)Quad->x,		   (float)Quad->y			 },	{ 999.0,  999.0},   {(float)Quad->w, (float)Quad->h, Quad->Rounding, Quad->Color} },
		{ { (float)Quad->x,		   (float)Quad->y + Quad->h },	{ 999.0, 1001.0},  {(float)Quad->w, (float)Quad->h, Quad->Rounding, Quad->Color} },
	};
...
void OpenVkInputText(size_t* Length, char* Text, const char* KeyWords)
{
	uint32_t Lines = 0;
	size_t LastOffset = 0;
