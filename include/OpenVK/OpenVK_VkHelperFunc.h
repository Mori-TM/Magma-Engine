...
//FIX all "Dynamic" arrays should use "CMA" or "DynamicArray"
typedef struct
{
...
void VkQuerySwapChainSupport(VkPhysicalDevice PhysicalDevice)
{
	VkSwapChainSupportDetails* Details = &VkRenderer.SwapChainSupportDetails;
	...
	if (OpenVkCheckedRealloc(Details->FormatCount, FormatCount, sizeof(VkSurfaceFormatKHR), (void**)&Details->Formats, "Surface Formats") == OpenVkTrue)
	{
		Details->FormatCount = FormatCount;
		vkGetPhysicalDeviceSurfaceFormatsKHR(PhysicalDevice, VkRenderer.Surface, &Details->FormatCount, Details->Formats);
	}

	uint32_t PresentModeCount = 0;
	vkGetPhysicalDeviceSurfacePresentModesKHR(PhysicalDevice, VkRenderer.Surface, &PresentModeCount, NULL);
	if (OpenVkCheckedRealloc(Details->PresentModeCount, PresentModeCount, sizeof(VkPresentModeKHR), (void**)&Details->PresentModes, "Present Modes") == OpenVkTrue)
	{
		Details->PresentModeCount = PresentModeCount;
		vkGetPhysicalDeviceSurfacePresentModesKHR(PhysicalDevice, VkRenderer.Surface, &Details->PresentModeCount , Details->PresentModes);
	}
...
VkPresentModeKHR VkChooseSwapPresentMode(VkPresentModeKHR* AvailablePresentModes, uint32_t PresentCount)
{
	...
	*/
	for (uint32_t i = 0; i < PresentCount; i++)
		if (AvailablePresentModes[i] == VK_PRESENT_MODE_IMMEDIATE_KHR)//VK_PRESENT_MODE_MAILBOX_KHR
