...
OpenVkBool VkGetRaytracingFeatures(VkDeviceCreateInfo* DeviceCreateInfo)
{
	...
	DynamicArrayCreate(&VkRaytracer.GeometryStorage, sizeof(VkAccelerationStructureGeometryKHR), "OpenVk Raytracer, Geometry Tmp");
	DynamicArrayCreate(&VkRaytracer.InstanceStorage, sizeof(VkAccelerationStructureInstanceKHR), "OpenVk Raytracer, Instance Tmp");
...
//Find out how to use dynamic vertex/index buffers with raytracing
uint32_t VkCreateRaytracingGeometry(OpenVkRaytracingGeometryCreateInfo* Info)
{
	VkDeviceOrHostAddressConstKHR VertexBufferDeviceAddress;
	VkDeviceOrHostAddressConstKHR IndexBufferDeviceAddress;
	VkDeviceOrHostAddressConstKHR TransformBufferDeviceAddress;
	
	VkStaticBufferInfo* Vertex;
	Vertex = (VkStaticBufferInfo*)CMA_GetAt(&VkRenderer.StaticBuffers, Info->VertexBuffer);
	if (!Vertex) return OpenVkRuntimeError("Failed to find Vertex Buffer: %d with Count: %d", Info->VertexBuffer, Info->VertexCount);
	...
	VkStaticBufferInfo* Transform = (VkStaticBufferInfo*)CMA_GetAt(&VkRenderer.StaticBuffers, Info->TranformBuffer);
	...
	GeometryInfo.VertexCount = Info->VertexCount;
	GeometryInfo.IndexCount = Info->IndexCount;
...
uint32_t VkCreateBottomLevelAS(uint32_t GeometryCount, uint32_t* InGeometry, OpenVkBool AllowUpdate, uint32_t* OldBottomLevelAS)
{
	DynamicArrayClear(&VkRaytracer.GeometryStorage);

//	uint32_t NumTriangles = (Geometry->IndexCount != 0 ? Geometry->IndexCount / 3 : Geometry->VertexCount / 3);
	uint32_t NumTriangles = 0;

	for (uint32_t i = 0; i < GeometryCount; i++)
	{
		VkRaytracingGeometryInfo* Geometry = (VkRaytracingGeometryInfo*)CMA_GetAt(&VkRaytracer.Geometry, InGeometry[i]);
		if (Geometry != NULL)
		{
			DynamicArrayPush(&VkRaytracer.GeometryStorage, &Geometry->AccelerationStructureGeometry);
			NumTriangles += (Geometry->IndexCount != 0 ? Geometry->IndexCount / 3 : Geometry->VertexCount / 3);
		}			
	}

//	VkRaytracingGeometryInfo* Geometry = (VkRaytracingGeometryInfo*)CMA_GetAt(&VkRaytracer.Geometry, InGeometry);
//	if (Geometry == NULL)
//		return OpenVkRuntimeError("Failed to find Geometry: %d", InGeometry);
	...
	AccelerationStructureBuildGeometryInfo.geometryCount = VkRaytracer.GeometryStorage.Size;
	AccelerationStructureBuildGeometryInfo.pGeometries = (VkAccelerationStructureGeometryKHR*)VkRaytracer.GeometryStorage.Data;
	...
	if (OldBottomLevelAS)
	{
		VkAccelerationStructure* BLAS = (VkAccelerationStructure*)CMA_GetAt(&VkRaytracer.AccelerationStructures, *OldBottomLevelAS);
		BottomLevelAS = *BLAS;
	}
...
uint32_t VkCreateASInstance(OpenVkTransformMatrix Matrix, uint32_t InstanceCustomIndex, OpenVkBool TriangleFrontCCW, uint32_t BottomLevelAS)
{
	...
	if (AccelerationStructure == NULL)
		return OpenVkRuntimeError("Failed to find Bottom Level Acceleration Structure: %d", BottomLevelAS);
...
void VkDestroyASInstance(uint32_t Insance)
{
	CMA_Pop(&VkRaytracer.Instances, Insance);
}

OpenVkBool VkUpdateASInstance(OpenVkTransformMatrix Matrix, OpenVkBool TriangleFrontCCW, uint32_t BottomLevelAS, uint32_t Instance)
{
	...
	InstancePTR->flags = (TriangleFrontCCW ? VK_GEOMETRY_INSTANCE_TRIANGLE_FRONT_COUNTERCLOCKWISE_BIT_KHR : VK_GEOMETRY_INSTANCE_TRIANGLE_FACING_CULL_DISABLE_BIT_KHR);
	InstancePTR->accelerationStructureReference = AccelerationStructure->DeviceAddress;
...
OpenVkBool VkCreateTopLevelAS(uint32_t InstanceCount, uint32_t* Instances, uint32_t MaxPrimitiveCount, OpenVkBool AllowUpdate, uint32_t* OldTopLevelAS)
{
	...
	for (uint32_t i = 0; i < InstanceCount; i++)
	{
		...
		if (Instance != NULL)
			DynamicArrayPush(&VkRaytracer.InstanceStorage, Instance);
	}
	...
	VkCreateVkBufferExt(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
						...
						VkRaytracer.InstanceStorage.Size * sizeof(VkAccelerationStructureInstanceKHR), VkRaytracer.InstanceStorage.Data, &InstancesBuffer.Buffer, &InstancesBuffer.BufferMemory);
	...
	if (OldTopLevelAS)
	{
		VkAccelerationStructure* TLAS = (VkAccelerationStructure*)CMA_GetAt(&VkRaytracer.AccelerationStructures, *OldTopLevelAS);
		TopLevelAS = *TLAS;
	}

//	const uint32_t MaxPrimitiveCount = InstanceCount;
	VkAccelerationStructureBuildSizesInfoKHR AccelerationStructureBuildSizesInfo;
	...
	return CMA_Push(&VkRaytracer.AccelerationStructures, &TopLevelAS);
}

OpenVkBool VkDestroAccelerationStructure(uint32_t AccelerationStructure)
{
	VkAccelerationStructure* AS = (VkAccelerationStructure*)CMA_GetAt(&VkRaytracer.AccelerationStructures, AccelerationStructure);
	if (AS != NULL)
	{
		...
		CMA_Pop(&VkRaytracer.AccelerationStructures, AccelerationStructure);
		return OpenVkTrue;
	}
	
	return OpenVkRuntimeError("Failed to find Acceleration Structure to destroy: %d", AccelerationStructure);
}
...
uint32_t VkCreateRaytracingPipeline(uint32_t MaxPipelineRayRecursionDepth, uint32_t PipelineLayout, uint32_t ShaderCount, uint32_t* ShaderTypes, OpenVkFile* Shader)
{
	...
	for (uint32_t i = 0; i < ShaderCount; i++)
	{
		...
		if (VkCreateShaderModule(Shader[i], &ShaderModule) == OPENVK_ERROR)
			return OpenVkRuntimeError("Failed to Create Raytracing Shader Module: %d", i);
...
OpenVkBool VkTraceRays(OpenVkTraceRaysInfo* Info)
{
	const uint32_t HandleSizeAligned = OpenVkAlignedSize(VkRaytracer.RayTracingPipelineProperties.shaderGroupHandleSize, VkRaytracer.RayTracingPipelineProperties.shaderGroupHandleAlignment);

	VkStaticBufferInfo* RayBufferInfo = (VkStaticBufferInfo*)CMA_GetAt(&VkRenderer.StaticBuffers, Info->RaygenShader);
	VkStaticBufferInfo* MissBufferInfo = (VkStaticBufferInfo*)CMA_GetAt(&VkRenderer.StaticBuffers, Info->MissShader);
	VkStaticBufferInfo* HitBufferInfo = (VkStaticBufferInfo*)CMA_GetAt(&VkRenderer.StaticBuffers, Info->HitShader);

	if (RayBufferInfo == NULL)	return OpenVkRuntimeError("Failed to find Raygen Shader: %d", Info->RaygenShader);
	if (MissBufferInfo == NULL) return OpenVkRuntimeError("Failed to find Miss Shader: %d", Info->MissShader);
	if (HitBufferInfo == NULL)	return OpenVkRuntimeError("Failed to find Hit Shader: %d", Info->HitShader);

	VkStridedDeviceAddressRegionKHR RaygenShaderSbtEntry;
	RaygenShaderSbtEntry.deviceAddress = VkGetBufferDeviceAddress(RayBufferInfo->Buffer);
	RaygenShaderSbtEntry.stride = HandleSizeAligned;
	RaygenShaderSbtEntry.size = HandleSizeAligned * Info->RaygenHandleCount;

	VkStridedDeviceAddressRegionKHR MissShaderSbtEntry;
	MissShaderSbtEntry.deviceAddress = VkGetBufferDeviceAddress(MissBufferInfo->Buffer);
	MissShaderSbtEntry.stride = HandleSizeAligned;
	MissShaderSbtEntry.size = HandleSizeAligned * Info->MissHandleCount;

	VkStridedDeviceAddressRegionKHR HitShaderSbtEntry;
	HitShaderSbtEntry.deviceAddress = VkGetBufferDeviceAddress(HitBufferInfo->Buffer);
	HitShaderSbtEntry.stride = HandleSizeAligned;
	HitShaderSbtEntry.size = HandleSizeAligned * Info->HitHandleCount;
...
{
	for (uint32_t i = 0; i < VkRaytracer.AccelerationStructures.Size; i++)
	{
		VkAccelerationStructure* AS = (VkAccelerationStructure*)CMA_GetAt(&VkRaytracer.AccelerationStructures, i);
		if (AS != NULL)
		{
			vkFreeMemory(VkRenderer.Device, AS->Memory, NULL);
			vkDestroyBuffer(VkRenderer.Device, AS->Buffer, NULL);
...
//	}
	...
	CMA_Destroy(&VkRaytracer.AccelerationStructures);
//	CMA_Destroy(&VkRaytracer.TopLevelAS);
	CMA_Destroy(&VkRaytracer.Instances);
	CMA_Destroy(&VkRaytracer.Geometry);
	DynamicArrayDestroy(&VkRaytracer.InstanceStorage);
	DynamicArrayDestroy(&VkRaytracer.GeometryStorage);
