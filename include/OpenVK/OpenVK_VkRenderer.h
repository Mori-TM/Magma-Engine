...
OpenVkBool VkCreateSwapChain(uint32_t* Width, uint32_t* Height)
{
	...
	VkQuerySwapChainSupport(VkRenderer.PhysicalDevice);
	VkSwapChainSupportDetails* SwapChainSupport = &VkRenderer.SwapChainSupportDetails;
	...
	*Height = Extent.height;
	...
	if (OpenVkCheckedRealloc(VkRenderer.SwapChainImageCount, ImageCount, sizeof(VkImage), (void**)&VkRenderer.SwapChainImages, "SwapChainImages") == OpenVkTrue)
...
//	vkGetSwapchainImagesKHR(VkRenderer.Device, VkRenderer.SwapChain, &ImageCount, VkRenderer.SwapChainImages);
	...
	if (OpenVkCheckedRealloc(VkRenderer.SwapChainImageCount, ImageCount, sizeof(VkImageView), (void**)&VkRenderer.SwapChainImageViews, "SwapChainImageViews") == OpenVkTrue)
	{
...
//	const char* DeviceExtensions[] = { VK_KHR_SWAPCHAIN_EXTENSION_NAME };
	...
	if (vkCreateInstance(&InstanceCreateInfo, NULL, &VkRenderer.Instance) != VK_SUCCESS)
		return OpenVkRuntimeError("Failed to Create Instance. Propably because Validation Layers are Enabled and not supported");
...
uint32_t VkCreateRenderPass(uint32_t AttachmentCount, uint32_t* Attachments, uint32_t* AttachmentFormats, uint32_t* AttachDesc, uint32_t* MsaaSamples, uint32_t RenderPassOptions)
{
	...
	for (uint32_t i = 0; i < AttachmentCount; i++)
	{
		...
		//	Attachment->finalLayout = ((Samples > 1) ? (RenderPassOptions & OPENVK_RENDER_PASS_RESOLVE_ATTACHMENTS ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) : (RenderPassOptions & OPENVK_RENDER_PASS_SAMPLED ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_PRESENT_SRC_KHR));
			...
			{
				if (RenderPassOptions & OPENVK_RENDER_PASS_SAMPLED)
					Attachment->finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
				else
			...
			if (RenderPassOptions & OPENVK_RENDER_PASS_RESOLVE_ATTACHMENTS)
			{		
				//FIX ? - Add AttachmentDescription like load operation load and load operation clear attachment - was brunder?
...
uint32_t VkCreateRenderPassExt(uint32_t AttachmentCount, VkAttachmentDescription* AttachmentDescriptions, uint32_t SubpassCount, VkSubpassDescription* Subpasses, uint32_t DependencyCount, VkSubpassDependency* Dependencies)
{
	VkRenderer.RenderPasses = (VkRenderPass*)OpenVkRealloc(VkRenderer.RenderPasses, (VkRenderer.RenderPassCount + 1) * sizeof(VkRenderPass));
...
void VkNextSubpass()
{
	vkCmdNextSubpass(VkRenderer.CommandBuffers[VkRenderer.ImageIndex], VK_SUBPASS_CONTENTS_INLINE);
}
...
//	VkRenderer.Pipelines = (VkPipeline*)OpenVkRealloc(VkRenderer.Pipelines, (VkRenderer.PipelineCount + 1) * sizeof(VkPipeline));
	...
	{
		AttributeDescriptions = (VkVertexInputAttributeDescription*)OpenVkMalloc(AttributeDescriptionCount * sizeof(VkVertexInputAttributeDescription));
...
//	OpenVkRuntimeWarning("Depth Clamping changed, check if evrything works");
	...
	DepthStencilState.depthWriteEnable = Info->DepthWrite;//VK_TRUE
	DepthStencilState.depthCompareOp = (VkCompareOp)Info->DepthCompare;//VK_COMPARE_OP_LESS_OR_EQUAL
...
uint32_t VkUpdateDescriptorSet(OpenVkDescriptorSetCreateInfo* Info)
{
	...
	if (ImageCount > 0)														//FIX - Quick and dirty solution * 3 I guess, why 3?
...
OpenVkBool VkDrawFrame(void (*RenderFunc)(void), void (*ResizeFunc)(OpenVkBool RecreateSwapChain), void (*UpdateFunc)(void))
{
	...
	VkResult AcquireImageResult = vkAcquireNextImageKHR(VkRenderer.Device, VkRenderer.SwapChain, UINT64_MAX, VkRenderer.ImageAvailableSemaphores[VkRenderer.CurrentFrame], VK_NULL_HANDLE, &VkRenderer.ImageIndex);
	...
	{
		return OpenVkRuntimeError("Failed acquire next image: %d", AcquireImageResult);
	}
...
//		return OpenVkRuntimeError("Failed acquire next image: %d", AcquireImageResult);
	...
	VkResult QueueResult = vkQueueSubmit(VkRenderer.GraphicsQueue, 1, (VkSubmitInfo*)&SubmitInfo, VkRenderer.InFlightFences[VkRenderer.CurrentFrame]);
//	exit(1);
	...
	QueueResult = vkQueuePresentKHR(VkRenderer.PresentQueue, &PresentInfo);
...
uint32_t VkCreateTexture(OpenVkTextureCreateInfo* Info)
{
	...
	vkGetPhysicalDeviceFormatProperties(VkRenderer.PhysicalDevice, TextureImage.Format, &FormatProperties);
	if (!(FormatProperties.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT))
	{
		OpenVkRuntimeWarning("No Linear Filter, No Mipmaps");
		VkRenderer.MipLevels = 1;
	}
...
uint32_t VkCreateImageSampler(uint32_t Filter, uint32_t AddressMode)
{
	...
	SamplerInfo.maxAnisotropy = VkRenderer.PhysicalDeviceProperties.limits.maxSamplerAnisotropy / 2;
...
void VkBindDescriptorSet(uint32_t PipelineLayout, uint32_t Set, uint32_t DescriptorSet, uint32_t PipelineType)
{
	...
	else OpenVkRuntimeWarning("Failed to find Descriptor Set: %d, binding wont be possible", DescriptorSet);
}
...
void VkDestroyRenderer()
{
	...
	OpenVkFree(VkRenderer.SwapChainSupportDetails.PresentModes);
	OpenVkFree(VkRenderer.SwapChainSupportDetails.Formats);
