...
#else
	//This function doesn't work on my maschine with clang
	long i;
	float x2, y;
...
{
	float length = FastInverseSqrt(v.x * v.x + v.y * v.y);
	v.x *= length;
	v.y *= length;
	return v;
}
...
#else
	float length = FastInverseSqrt(v->x * v->x + v->y * v->y + v->z * v->z + v->w * v->w);
	v->x *= length;
	v->y *= length;
	v->z *= length;
	v->w *= length;
...
mat4 LookAtMat4PFast(vec4* Pos, vec4* LookAt, vec4* Up)
{
	...
	Vec4P(X.x, Y.x, Z.x, 0.0, &Matrix.m4[0].Fast);
	Vec4P(X.y, Y.y, Z.y, 0.0, &Matrix.m4[1].Fast);
	Vec4P(X.z, Y.z, Z.z, 0.0, &Matrix.m4[2].Fast);
	Vec4P(-Dot4P(&X, Pos), -Dot4P(&Y, Pos), -Dot4P(&Z, Pos), 1.0, &Matrix.m4[3].Fast);
...
void PerspectiveMat4PFast(float Aspect, float FOV, float NearPlane, float FarPlane, mat4* m)
{
	...
	Vec4P(F / Aspect, 0.0, 0.0, 0.0, &m->m4[0].Fast);
	Vec4P(0.0, -F, 0.0, 0.0, &m->m4[1].Fast);
	Vec4P(0.0, 0.0, FarPlane / (NearPlane - FarPlane), -1.0, &m->m4[2].Fast);
	Vec4P(0.0, 0.0, (NearPlane * FarPlane) / (NearPlane - FarPlane), 0.0, &m->m4[3].Fast);
}
...
{
	Vec4P(2 / (Right - Left), 0.0, 0.0, 0.0, &m->m4[0].Fast);
	Vec4P(0.0, 2 / (Top - Bottom), 0.0, 0.0, &m->m4[1].Fast);
	Vec4P(0.0, 0.0, -1 / (FarZ - NearZ), 0.0, &m->m4[2].Fast);
	Vec4P(-(Right + Left) / (Right - Left), -(Top + Bottom) / (Top - Bottom), -NearZ / (FarZ - NearZ), 1.0, &m->m4[3].Fast);
}
...
mat4 TransposeMat4(mat4* Mat)
{
	...
	Vec4P(Mat->m[0][0], Mat->m[1][0], Mat->m[2][0], Mat->m[3][0], &Matrix.m4[0].Fast);
	Vec4P(Mat->m[0][1], Mat->m[1][1], Mat->m[2][1], Mat->m[3][1], &Matrix.m4[1].Fast);
	Vec4P(Mat->m[0][2], Mat->m[1][2], Mat->m[2][2], Mat->m[3][2], &Matrix.m4[2].Fast);
	Vec4P(Mat->m[0][3], Mat->m[1][3], Mat->m[2][3], Mat->m[3][3], &Matrix.m4[3].Fast);
