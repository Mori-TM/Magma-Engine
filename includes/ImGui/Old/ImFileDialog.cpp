...
namespace ifd {
	...
	/* UI CONTROLS */
	bool FolderNode(const char* label, ImageData icon, bool& clicked)
	{
		...
		if (icon.DescriptorSet != 0)
			window->DrawList->AddImage(&GetDescriptorSet(icon.DescriptorSet)[0], ImVec2(icon_posX, pos.y), ImVec2(icon_posX + ICON_SIZE, pos.y + ICON_SIZE));
		else
			window->DrawList->AddRect(ImVec2(icon_posX, pos.y), ImVec2(icon_posX + ICON_SIZE, pos.y + ICON_SIZE), ImGui::GetColorU32(ImVec4(1, 1, 1, 1)));
	...
	bool FileIcon(const char* label, bool isSelected, ImTextureID icon, ImVec2 size, bool hasPreview, int previewWidth, int previewHeight, bool RenderImage)
	{
		...
		if (hasPreview) {
			...
			if (RenderImage)
				...
				window->DrawList->AddRect(ImVec2(previewPosX, previewPosY), ImVec2(previewPosX + availSize.x, previewPosY + availSize.y), ImGui::GetColorU32(ImVec4(1, 1, 1, 1)));
		}
		else
		{
			if (RenderImage)
				...
				window->DrawList->AddRect(ImVec2(iconPosX, pos.y), ImVec2(iconPosX + iconSize, pos.y + iconSize), ImGui::GetColorU32(ImVec4(1, 1, 1, 1)));
		}
	...
	ImageData FileDialog::m_getIcon(const std::filesystem::path& path)
	{
#ifdef _WIN32
...
//		m_clearIconPreview();
	...
	ImageData FileDialog::m_getIconNew(std::string Tmp)
	{
		if (Tmp[0] != 'C')
			Tmp.insert(2, "\\");

	//	std::filesystem::path PathTmp = Tmp;
		return m_getIcon(Tmp);
	}
	...
	//	if (m_getIcon(node->Path).DescriptorSet == 0)
	//		printf("Path: %s\n", node->Path.root_path().c_str());
		...
		*/
		...
		if (FolderNode(displayName.c_str(), m_getIconNew(node->Path.string()), isClicked)) {
	...
	void FileDialog::m_renderContent()
	{
		...
		if (m_zoom == 1.0f) {
			if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN))) {
				...
				for (auto& entry : m_content) {
					...
					ImageData Data = m_getIconNew(entry.Path.string());

					if (Data.DescriptorSet != 0)
						ImGui::Image(&GetDescriptorSet(Data.DescriptorSet)[0], ImVec2(ICON_SIZE, ICON_SIZE));
					else
						ImGui::Button(" ", ImVec2(ICON_SIZE, ICON_SIZE));
					ImGui::SameLine();
		...
		// "icon" view
			...
			for (auto& entry : m_content) {
				...
				{
					if (entry.IconPreview.DescriptorSet != 0)
						RenderImage = true;					
				}
				else
					if (Data.DescriptorSet != 0)
			...
			//	RenderImage = false;
				...
				if (FileIcon(filename.c_str(), isSelected, entry.HasIconPreview ? &GetDescriptorSet(entry.IconPreview.DescriptorSet)[0] : &GetDescriptorSet(Data.DescriptorSet)[0], ImVec2(32 + 16 * m_zoom, 32 + 16 * m_zoom), entry.HasIconPreview, entry.IconPreviewWidth, entry.IconPreviewHeight, RenderImage)) {
					std::error_code ec;
					bool isDir = std::filesystem::is_directory(entry.Path, ec);
