...
typedef struct
{
	...
	uint32_t TextureDescriptorSet;//FIX - Never gets freed in some way when texture is deleted!!!!
} SceneTextureImage;
...
//	uint32_t DescriptorSet;//This gets create once for and with every mesh add, further modifications only via descriptor updates
...
typedef struct
{
	char Line[MAX_CHAR_LINE_LENGTH];
} SceneScriptLine;
...
typedef struct
{
	...
	uint32_t LineCount;
	uint32_t AllocatedLineCount;
	

	SceneScriptLine* Lines;
//	char* Lines[MAX_CHAR_LINE_LENGTH];
