...
void CreateSwapChainPipeline()
{
	...
	GraphicsPipelineCreateInfo.Width = MainHWnd.Width;
	GraphicsPipelineCreateInfo.Height = MainHWnd.Height;
...
void CreateSwapChainFramebuffer()
{
	...
	FramebufferCreateInfo.Width = MainHWnd.Width;
	FramebufferCreateInfo.Height = MainHWnd.Height;
...
void SwapChainDraw()
{
	...
	BeginInfo.x = 0;
	BeginInfo.y = 0;
	BeginInfo.Width = MainHWnd.Width;
	BeginInfo.Height = MainHWnd.Height;
	OpenVkBeginRenderPass(&BeginInfo);
	{
		...
		if (RenderGamePreview)
		{
			...
			OpenVkSetScissor(0, 0, MainHWnd.Width, MainHWnd.Height);
			OpenVkSetViewport(0, 0, MainHWnd.Width, MainHWnd.Height);
		...
		else
		{
			...
			VkDescriptorSet* DescriptorSet = GetDescriptorSet(SceneRenderDescriptorSet);
			if (DescriptorSet)
				OpaqueTextures[0] = (ImTextureID*)&DescriptorSet[VkRenderer.CurrentFrame];

			ImGui_ImplVulkan_RenderDrawData(ImGui::GetDrawData(), VkRenderer.CommandBuffers[VkRenderer.ImageIndex], DescriptorSet == NULL ? 0 : 1, OpaqueTextures);

		}	
		if (GamePerformanceOverlay)
	//	if (1 == 0)
		{
			OpenVkGUIBeginRender(MainHWnd.Width, MainHWnd.Height);
			{
			...
			//	String[3] = 0;
				...
				/*
					...
					//	OpenVkGUIText(String);
						...
						OpenVkInputText(&DummyScriptLength, DummyScript, "function0,GetEntity1");
					...
					OpenVkGUIBegin("Another Window2");
					{
						//	OpenVkGUIText(String);
