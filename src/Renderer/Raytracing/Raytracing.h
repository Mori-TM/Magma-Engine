...
//	RTR.TransformBuffers	= DynamicArrayCreate(sizeof(uint32_t), "Transform Buffers");
	...
	RTR.StorageImage = OpenVkCreateStorageImage(MainHWnd.Width, MainHWnd.Height, OPENVK_FORMAT_DEFAULT);
...
uint32_t RaytracingAddMesh(uint32_t SceneMeshIndex)
{
		...
		
	{
		...
		uint32_t Instance = OpenVkCreateASInstance(ModelOVK, OpenVkTrue, *BottomLevelAS);
